...
if torch.cuda.is_available():
    #device = 'cuda'
    gpu_id = 1  # Change this ID to the desired GPU
...
def load(drop_correlated = True, corr_thr = 0.85, feature_files=[WAVLM_IMAGEBIND_FEATURES_FILE]):
    ...
    print('Null Dropped from the Dataset ... ... ...')
    '''
    ...
    '''
    if drop_correlated:
        print('Co-relation Analysis Started ... ... ...')
    ...
    print('Done with Co-relation Analysis ... ... ...')
    features = df.loc[:, df_features.columns[0]:df_features.columns[-1]]
    columns = features.columns
    print('Feature Column Count:', len(columns))
...
def evaluate(model, dataloader, num_trials, num_buckets):
    ...
    wrapped_model = ModelWrapper(model,criterion,replicate_in_memory=False)
...
@click.option("--dropout_prob", default=0.18775382439425725)
@click.option("--num_trials", default=100, help="Options: 100, 500, 1000, 5000, 10000, 50000")
@click.option("--num_buckets", default=20, help="Options: 5, 10, 20, 50, 100")
@click.option("--learning_rate", default=0.8929821311651904, help="Learning rate for classifier")
...
@click.option("--num_epochs", default=97)
@click.option("--drop_correlated", default='no', help="Options: yes, no")
@click.option("--corr_thr", default=0.9)
...
@click.option("--momentum", default=0.841759056939497)
@click.option("--use_scheduler", default='no', help="Options: yes, no")
@click.option("--scheduler", default='reduce', help="Options: step, reduce")
@click.option("--step_size", default=25)
@click.option("--gamma", default=0.9257856450324876)
@click.option("--patience", default=4)

def main(**cfg):
    ENABLE_WANDB = False
    if ENABLE_WANDB:
        wandb.init(project="park_fda_experiments", config=cfg, entity="roc-hci")
    ...
    print('Total PD samples:', sum(labels), round(sum(labels) / len(features) * 100, 1), '%')
    print('Total Unique IDs:', len(set(ids)))

    # Get unique IDs where label == 1 (PD)
    pd_ids = {id_ for id_, label in zip(ids, labels) if label == 1}
    print('Total Unique IDs with PD:', len(pd_ids), round(len(pd_ids)/len(set(ids))*100, 1), '%')
    ...
    print('Total Samples:', len(features_train))
    print('Total PD samples:', sum(labels_train), round(sum(labels_train) / len(features_train) * 100, 1), '%')
    print('Total Unique IDs:', len(set(ids_train)))

    # Get unique IDs where label == 1 (PD)
    pd_ids = {id_ for id_, label in zip(ids_train, labels_train) if label == 1}
    print('Total Unique IDs with PD:', len(pd_ids), round(len(pd_ids)/len(set(ids_train))*100, 1), '%')
    
    print('\n----------------------------------------------------\n')
    
    print('Development Dataset Summary')
    print('Total Samples:', len(features_dev))
    print('Total PD samples:', sum(labels_dev), round(sum(labels_dev) / len(features_dev) * 100, 1), '%')
    print('Total Unique IDs:', len(set(ids_dev)))

    # Get unique IDs where label == 1 (PD)
    pd_ids = {id_ for id_, label in zip(ids_dev, labels_dev) if label == 1}
    print('Total Unique IDs with PD:', len(pd_ids), round(len(pd_ids)/len(set(ids_dev))*100, 1), '%')
    
    print('\n----------------------------------------------------\n')
    
    print('Test Dataset Summary')
    print('Total Samples:', len(features_test))
    print('Total PD samples:', sum(labels_test), round(sum(labels_test) / len(features_test) * 100, 1), '%')
    print('Total Unique IDs:', len(set(ids_test)))

    # Get unique IDs where label == 1 (PD)
    pd_ids = {id_ for id_, label in zip(ids_test, labels_test) if label == 1}
    print('Total Unique IDs with PD:', len(pd_ids), round(len(pd_ids)/len(set(ids_test))*100, 1), '%')
    ...
    for epoch in tqdm(range(cfg['num_epochs'])):
        ...
        if dev_loss < best_dev_loss:
            ...
            if ENABLE_WANDB:
                wandb.log({'running_best_dev_loss':best_dev_loss})
    ...
    print(f'From Saved Model: {MODEL_PATH}')
    # '''
