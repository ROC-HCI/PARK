...
id_to_debug = 'q6V93GajKUbF0qlaMZZ634PyTiC2'
...
with open(os.path.join(BASE_DIR,"data/dev_set_participants.txt")) as f:
    ...
    print('Length of Dev IDs', len(dev_ids))

with open(os.path.join(BASE_DIR,"data/test_set_participants.txt")) as f:
    ids = f.readlines()
    test_ids = set([x.strip() for x in ids])
    print('Length of Total Test IDs', len(test_ids))

with open(os.path.join(BASE_DIR,"data/test_set_participants_validation.txt")) as f:
    ids = f.readlines()
    test_ids_validation_1 = set([x.strip() for x in ids])
    print('Length of Validation 1 Test IDs', len(test_ids_validation_1))

with open(os.path.join(BASE_DIR,"data/test_set_participants_validation_2.txt")) as f:
    ids = f.readlines()
    test_ids_validation_2 = set([x.strip() for x in ids])
    print('Length of Validation 2 Test IDs', len(test_ids_validation_2))
    
with open(os.path.join(BASE_DIR,"data/test_set_participants_others.txt")) as f:
    ids = f.readlines()
    test_ids_global = set([x.strip() for x in ids]) 
    print('Length of Balanced Test IDs', len(test_ids_global))
...
def load_smile_data(drop_correlated = True, corr_thr = 0.85):
    ...
    # Convert to datetime format and format as YYYY-MM-DD
    ...
    print( '---------------------- Data for smile is loaded ----------------------')
    print( '---------------------- -------------- ----------------------')
    print(len(columns))
    print( '---------------------- -------------- ----------------------')
...
def load_qbf_data(drop_correlated = False, corr_thr = 0.85, feature_files=[AUDIO_FEATURES_FILE]):
    ...
    for column in df.columns:
        if 'imagebind' in column:
            image_bind_columns.append(column)
            
    df.drop(columns=image_bind_columns, inplace=True) 
    ...
    df["id_date"] = df["id"]+"#"+df["date"]
    df["label"] = df["pd"]
    print( '---------------------- Data for speech is loaded ----------------------')
    print( '---------------------- -------------- ----------------------')
    print(len(columns))
    print( '---------------------- -------------- ----------------------')
    return features, df["label"], df["id"], columns, df["id_date"]
...
def load_finger_data(hand="left",drop_correlated = False, corr_thr = 0.85):
    ...
    df["id_date"] = df["id"]+"#"+df["date"]
    df["label"] = 1.0*(df["pd"]!="no")
    print( f'---------------------- Data for {hand} hand finger tapping is loaded ----------------------')
    print( '---------------------- -------------- ----------------------')
    print(len(columns))
    print( '---------------------- -------------- ----------------------')
    return features, df["label"], df["id"], columns, df["id_date"]
...
def train_test_split(df):
    ...
    test_df_validation_1 = df[df["id"].isin(test_ids_validation_1)]
    test_df_validation_2 = df[df["id"].isin(test_ids_validation_2)]
    test_df_global = df[df["id"].isin(test_ids_global)]
    return train_df, test_df, test_df_validation_1, test_df_validation_2, test_df_global
...
def compute_metrics(y_true, y_pred_scores, threshold = 0.5):
    ...
    # make a dataframe with the predictions and labels and save it
    df = pd.DataFrame({"labels":labels, "preds":preds, "pred_scores":pred_scores})
...
def evaluate_fusion_model(fusion_model, dataloader, prediction_models, config, split="dev"):
    ...
    if "test" in split:
        # print('Test Set lenght:', len(all_labels))
        # print('Length of all_pred_scores:', len(all_pred_scores))
        coverage = (len(all_labels) - uncertain_indices.sum())/len(all_labels)
        with open(f"{split}_{config['seed']}.pkl", 'wb') as f:
            pickle.dump(
                {
        ...
        print('length of uncertain indices:', uncertain_indices.sum())
        all_labels = all_labels[~uncertain_indices]
        all_final_predictions = all_final_predictions[~uncertain_indices]
        pd.DataFrame(uncertain_indices).to_csv('uncertain_indices.csv', index=False)
...
@click.option("--dropout_prob", default=0.1932168363806878)
@click.option("--num_buckets", default=50, help="Options: 5, 10, 20, 50, 100")
@click.option("--num_trials", default=30, help="Options: 100-1000")
@click.option("--uncertainty_weight", default=37.155919411297965)
@click.option("--minority_oversample", default="no", help="Options: 'yes', 'no'")
@click.option("--sampler", default="KMeansSMOTE", help="Options: SMOTE, SMOTENC, SVMSMOTE, ADASYN, BorderlineSMOTE, KMeansSMOTE, SMOTEN, RandomOverSampler, SMOTEENN, SMOTETomek")
@click.option("--train_random_noise", default="no", help="Options: yes, no")
@click.option("--validation_random_noise", default="no", help="Options: yes, no")
@click.option("--increase_variance", default="no", help="Options: yes, no")
@click.option("--temperature", default=0.05, help="Float between 0 and 1")
...
@click.option("--random_state", default=357, help="Random state for classifier")
@click.option("--model_subset_choice", default=0, help="4 possible choices. See Constants.py")
@click.option("--seed", default=289, help="Seed for random")
@click.option("--batch_size", default=1024)
@click.option("--num_epochs", default=285)
@click.option("--hidden_dim", default=8)
@click.option("--query_dim", default=32)
@click.option("--last_hidden_dim", default=128)
@click.option("--optimizer", default="SGD", help="Options: SGD, AdamW, RMSprop")
@click.option("--beta1", default=0.9)
@click.option("--beta2", default=0.999)
@click.option("--weight_decay", default=0.0001)
@click.option("--momentum", default=0.6720094670233973)
@click.option("--use_scheduler", default="no", help="Options: yes, no")
@click.option("--scheduler", default="reduce", help="Options: step, reduce")
@click.option("--step_size", default=23)
@click.option("--gamma", default=0.5453684198300217)
...
def main(**cfg):
    ...
    df.to_csv(BASE_DIR + '/data/full_fusion_dataset.csv', index = False)
    
    print("Data of finger tapping, audio, and smile is combined and loaded.")
    
    train_df, test_df, test_df_validation_1, test_df_validation_2, test_df_global = train_test_split(df)
    
    train_df.to_csv(BASE_DIR + '/data/train_val_data_to_save.csv', index = False)

    
    train_df, dev_df = train_dev_split(train_df)
    
    test_df_to_save = test_df[['row_id', 'id', 'label']]
    test_df_to_save.to_csv(BASE_DIR + '/data/test_data_to_save.csv', index = False)
    
    test_df_validation_1_to_save = test_df_validation_1[['row_id', 'id', 'label']]
    test_df_validation_1_to_save.to_csv(BASE_DIR + '/data/test_df_validation_1_to_save.csv', index = False)
    
    test_df_validation_2_to_save = test_df_validation_2[['row_id', 'id', 'label']]
    test_df_validation_2_to_save.to_csv(BASE_DIR + '/data/test_df_validation_2_to_save.csv', index = False)
    
    test_df_global_to_save = test_df_global[['row_id', 'id', 'label']]
    test_df_global_to_save.to_csv(BASE_DIR + '/data/test_df_global_to_save.csv', index = False)
    
    print('\n----------------------------------------------------\n')
    print('Full Dataset Set Summary')
    print('Total Samples:', len(df))
    print('Total PD samples:', len(df[df['label']==1.0]), round(len(df[df['label']==1.0])/len(df)*100, 1),'%')
    print('Total Unique IDs:', len(set(df['id'].to_list())))
    print('Total PD Unique IDs:', len(set(df[df['label']==1.0]['id'].to_list())), round(len(set(df[df['label']==1.0]['id'].to_list()))/ len(set(df['id'].to_list()))*100, 1))
    
    print('\n----------------------------------------------------\n')
    print('Training Set Summary')
    print('Total Samples:', len(train_df))
    print('Total PD samples:', len(train_df[train_df['label']==1.0]), round(len(train_df[train_df['label']==1.0])/len(train_df)*100, 1),'%')
    print('Total Unique IDs:', len(set(train_df['id'].to_list())))
    print('Total PD Unique IDs:', len(set(train_df[train_df['label']==1.0]['id'].to_list())), round(len(set(train_df[train_df['label']==1.0]['id'].to_list()))/ len(set(train_df['id'].to_list()))*100, 1))
    print('\n----------------------------------------------------\n')
    print('Development Set Summary')
    print('Total Samples:', len(dev_df))
    print('Total PD samples:', len(dev_df[dev_df['label']==1.0]), round(len(dev_df[dev_df['label']==1.0])/len(dev_df)*100, 1),'%')
    print('Total Unique IDs:', len(set(dev_df['id'].to_list())))
    print('Total PD Unique IDs:', len(set(dev_df[dev_df['label']==1.0]['id'].to_list())), round(len(set(dev_df[dev_df['label']==1.0]['id'].to_list()))/ len(set(dev_df['id'].to_list()))*100, 1))
    print('\n----------------------------------------------------\n')
    print('Test Set Summary')
    print('Total Samples:', len(test_df))
    print('Total PD samples:', len(test_df[test_df['label']==1.0]), round(len(test_df[test_df['label']==1.0])/len(test_df)*100, 1),'%')
    print('Total Unique IDs:', len(set(test_df['id'].to_list())))
    print('Total PD Unique IDs:', len(set(test_df[test_df['label']==1.0]['id'].to_list())), round(len(set(test_df[test_df['label']==1.0]['id'].to_list()))/ len(set(test_df['id'].to_list()))*100, 1))
    print('\n----------------------------------------------------\n')
    
    print('Balanced Test Set Summary')
    print('Total Samples:', len(test_df_global))
    print('Total PD samples:', len(test_df_global[test_df_global['label']==1.0]), round(len(test_df_global[test_df_global['label']==1.0])/len(test_df_global)*100, 1),'%')
    print('Total Unique IDs:', len(set(test_df_global['id'].to_list())))
    print('Total PD Unique IDs:', len(set(test_df_global[test_df_global['label']==1.0]['id'].to_list())), round(len(set(test_df_global[test_df_global['label']==1.0]['id'].to_list()))/ len(set(test_df_global['id'].to_list()))*100, 1))
    print('\n----------------------------------------------------\n')
    
    print('Validation Study 1 Test Set Summary')
    print('Total Samples:', len(test_df_validation_1))
    print('Total PD samples:', len(test_df_validation_1[test_df_validation_1['label']==1.0]), round(len(test_df_validation_1[test_df_validation_1['label']==1.0])/len(test_df_validation_1)*100, 1),'%')
    print('Total Unique IDs:', len(set(test_df_validation_1['id'].to_list())))
    print('Total PD Unique IDs:', len(set(test_df_validation_1[test_df_validation_1['label']==1.0]['id'].to_list())), round(len(set(test_df_validation_1[test_df_validation_1['label']==1.0]['id'].to_list()))/ len(set(test_df_validation_1['id'].to_list()))*100, 1))
    print('\n----------------------------------------------------\n')
    
    print('Validation Study 2 Test Set Summary')
    print('Total Samples:', len(test_df_validation_2))
    print('Total PD samples:', len(test_df_validation_2[test_df_validation_2['label']==1.0]), round(len(test_df_validation_2[test_df_validation_2['label']==1.0])/len(test_df_validation_2)*100, 1),'%')
    print('Total Unique IDs:', len(set(test_df_validation_2['id'].to_list())))
    print('Total PD Unique IDs:', len(set(test_df_validation_2[test_df_validation_2['label']==1.0]['id'].to_list())), round(len(set(test_df_validation_2[test_df_validation_2['label']==1.0]['id'].to_list()))/ len(set(test_df_validation_2['id'].to_list()))*100, 1))
    print('\n----------------------------------------------------\n')
    ...
    test_dataset_validation_1 = TensorDataset(test_df_validation_1)
    test_dataset_validation_2 = TensorDataset(test_df_validation_2)
    test_dataset_global = TensorDataset(test_df_global)
    

    train_loader = DataLoader(train_dataset, batch_size=cfg['batch_size'], shuffle=True)
    dev_loader = DataLoader(dev_dataset, batch_size=cfg["batch_size"])
    test_loader = DataLoader(test_dataset, batch_size = cfg['batch_size'])
    test_loader_validation_1 = DataLoader(test_dataset_validation_1, batch_size = cfg['batch_size'])
    test_loader_validation_2 = DataLoader(test_dataset_validation_2, batch_size = cfg['batch_size'])
    test_loader_global = DataLoader(test_dataset_global, batch_size = cfg['batch_size'])
    ...
    test_metrics_validation_1 = evaluate_fusion_model(best_model, test_loader_validation_1, prediction_models, cfg, split="test_df_validation_1_to_save")
    
    print("Validation 1 Results")
    print(test_metrics_validation_1)
    
    test_metrics_validation_2 = evaluate_fusion_model(best_model, test_loader_validation_2, prediction_models, cfg, split="test_df_validation_2_to_save")
    
    print("Validation 2 Results")
    print(test_metrics_validation_2)
    
    test_metrics_global = evaluate_fusion_model(best_model, test_loader_global, prediction_models, cfg, split="test_df_global_to_save")
    
    print("Global Results")
    print(test_metrics_global)
