...
def extract_features(filename, output_path, hand, labels=(0,""), display=False):
    '''
    For the filename, create a folder and save the plots, MP annotations, and mid-level features there.
    return features as a dictionary

    hand: left, right
    display: if True, shows OpenCV windows (requires GUI); default False for headless runs
    '''
    hand = hand.lower()
    FEATURE_DIR = output_path
    annotations = {}
    ...
    base_file = os.path.basename(filename)
    # support .webm/.mp4/etc (find last dot)
    if "." in base_file:
        base_root = base_file[:base_file.rfind(".")]
    else:
        base_root = base_file

    full_dir_path = os.path.join(FEATURE_DIR, base_root)
    if not os.path.exists(full_dir_path):
        os.mkdir(full_dir_path)
    ...
    if os.path.exists(full_dir_path):
        pkl_path = os.path.join(full_dir_path, "intermediate_features.pkl")
        if os.path.exists(pkl_path):
            with open(pkl_path, 'rb') as handle:
                data = pickle.load(handle)
                features = get_final_features(data)
                return features
        # if dir exists but no pkl, proceed to process anew

    # Process a new file
    os.makedirs(full_dir_path, exist_ok=True)
    annotation_image_path = os.path.join(full_dir_path, "MP Annotation Frames")
    annotation_json_path = os.path.join(full_dir_path, "MP Annotation JSON")
    os.makedirs(annotation_image_path, exist_ok=True)
    ...
    D = []  # Time-series of angular distance
    W = []  # Time-seris of wrist coordinates (normalized): (W.x, W.y)
    NOT_FOUND = -1  # Constant
    NUM_FRAMES = 0  # Count the number of frames in the video
    out = None
    ...
    while True:
        ...
        if NUM_FRAMES == 1:
            out = cv.VideoWriter(os.path.join(full_dir_path, "output.mp4"),
                                 cv.VideoWriter_fourcc(*'mp4v'), 15, (width, height))
        ...
        if "WRIST" in landmarks.keys():
            ...
            Vector_WI = ((index_x - wrist_x), (index_y - wrist_y))
            dot = Vector_WT[0] * Vector_WI[0] + Vector_WT[1] * Vector_WI[1]
            denom = math.sqrt((Vector_WT[0] ** 2) + (Vector_WT[1] ** 2)) * math.sqrt((Vector_WI[0] ** 2) + (Vector_WI[1] ** 2))
            cosx = dot / denom if denom != 0 else 1.0
            cosx = np.minimum(np.maximum(cosx, -1.0), 1.0)
            angle = (math.acos(cosx) * 180) / math.pi
...
def extract_participant_id_and_hand_and_date(filename):
    """
    Extracts participant_id, hand, and date from a filename following a specific pattern.
    If the pattern is not matched, returns filename as participant_id and today's date.

    Expected pattern example:
        prefix_<participant_id>_<something>_left_2024-04-02.mp4

    Args:
        filename (str): The name of the file.

    Returns:
        tuple: (participant_id, hand, date)
    """
    # Extract hand
    ...
    # Try to extract participant_id (assuming pattern has underscores)
    parts = filename.split('_')
    pattern_ok = len(parts) >= 3 and bool(re.search(r'\d{4}-\d{2}-\d{2}', filename))

    if pattern_ok:
        # Extract participant_id (second part, e.g., "P1234")
        participant_id = parts[1]
        # Extract date
        date_match = re.search(r'\d{4}-\d{2}-\d{2}', filename)
        date_str = date_match.group() if date_match else str(date.today())
    else:
        # Fallback if pattern not matched
        participant_id = filename
        date_str = str(date.today())

    return participant_id, hand, date_str


def load_labels_csv(labels_csv_path):
    """
    Load labels CSV into a dict keyed by base filename (without extension).
    Expected columns (flexible): filename, rating, diagnosis
    Returns dict: { base_name: (rating, diagnosis) }
    """
    ...
    # heuristic: find filename column
    fname_col = None
    for c in ["filename", "file", "name", "id"]:
        if c in df.columns:
            fname_col = c
            break
    if fname_col is None:
        raise ValueError("Labels CSV must include a filename column (e.g., 'filename').")
    ...
    for _, row in df.iterrows():
        raw = str(row[fname_col])
        base = raw
        if "." in raw:
            base = raw[:raw.rfind(".")]
        rating = row[rating_col] if rating_col else 0
        diagnosis = row[diag_col] if diag_col else ""
        label_map[base] = (rating, diagnosis)
    return label_map


def safe_hand_from_cli_or_filename(cli_hand, filename):
    """
    Resolve the hand to use.
    - If cli_hand is 'left' or 'right', return it.
    - If cli_hand is 'auto', try to parse from filename via extract_participant_id_and_hand_and_date.
      If not found, default to 'left' (common convention), but you can change here if desired.
    """
    if cli_hand in ("left", "right"):
        return cli_hand
    # auto
    pid, hand, dt = extract_participant_id_and_hand_and_date(os.path.basename(filename))
    if hand in ("left", "right"):
        return hand
    return "left"


def write_features_row(csv_path, row_dict, header_order=None):
    """
    Append a single features row to CSV (create if missing).
    header_order allows stable column ordering (list of keys).
    """
    ...
    with open(csv_file, "a", encoding="utf-8") as f:
        if write_header:
            ...
            f.write(",".join([str(h) for h in header_order]) + "\n")
        if header_order is None:
            header_order = list(row_dict.keys())
        line = ",".join([json.dumps(row_dict.get(k, "")) if isinstance(row_dict.get(k, ""), (dict, list))
                         else str(row_dict.get(k, "")) for k in header_order])
        f.write(line + "\n")


def collect_feature_row(features, filename, participant_id, hand, date_str, labels_tuple):
    """
    Prepare a flat dict row with metadata + features.
    """
    rating, diagnosis = labels_tuple if labels_tuple else (0, "")
    row = {
...
def iter_video_files(folder):
    """
    Yield paths to video files in folder with common extensions.
    """
    exts = (".webm", ".mp4", ".mov", ".avi", ".mkv", ".m4v")
    for p in Path(folder).rglob("*"):
        if p.is_file() and p.suffix.lower() in exts:
            yield str(p)
            
# in main we get the filename or a folder name and call the feature extractor
# if this is a filename (with -file), we extract the features and save them in a csv file
# if this is a folder name (with -folder), we go through all the files in the folder and extract
# for -folder, it should also provide -labels <csv-file-with-labels> containing labels by filenames 
def main():
    parser = argparse.ArgumentParser(description="Extract finger-tapping features with MediaPipe and save CSV.")
    mode = parser.add_mutually_exclusive_group(required=True)
    mode.add_argument("-file", "--file", dest="file", help="Path to a single video file.")
    mode.add_argument("-folder", "--folder", dest="folder", help="Path to a folder of video files (recursively).")

    parser.add_argument("-o", "--output", dest="output", required=True,
                        help="Output directory for intermediate files and CSV.")
    parser.add_argument("--csv", dest="csv", default="features.csv",
                        help="Output CSV filename (will be created in the output directory).")
    parser.add_argument("--labels", dest="labels", default=None,
                        help="Optional labels CSV with columns: filename, rating, diagnosis.")
    parser.add_argument("--hand", dest="hand", choices=["left", "right", "auto"], default="auto",
                        help="Which hand to process. 'auto' tries to infer from filename. Default: auto.")

    args = parser.parse_args()

    out_dir = Path(args.output)
    out_dir.mkdir(parents=True, exist_ok=True)
    out_csv_path = out_dir / args.csv

    labels_map = load_labels_csv(args.labels) if args.labels else {}

    def process_one(fp):
        base = os.path.basename(fp)
        # Parse participant_id / hand / date from filename; fallbacks handled inside the function
        participant_id, parsed_date = None, None
        try:
            participant_id, hand_inferred, parsed_date = extract_participant_id_and_hand_and_date(base)
        except Exception:
            # absolute fallback
            participant_id, hand_inferred, parsed_date = base, None, str(date.today())

        # Resolve hand preference (CLI overrides; 'auto' falls back to parsed)
        hand_to_use = safe_hand_from_cli_or_filename(args.hand, base if args.file else fp)

        # Resolve labels
        base_root = base[:base.rfind(".")] if "." in base else base
        labels_tuple = labels_map.get(base_root, (0, ""))

        # Run extraction
        feats = extract_features(fp, str(out_dir), hand_to_use, labels=labels_tuple)

        # Collect row and write to CSV
        row = collect_feature_row(feats, fp, participant_id, hand_to_use, parsed_date, labels_tuple)
        # Stable header: metadata first, then alphabetical features for reproducibility
        meta_keys = ["filename", "participant_id", "hand", "date", "rating", "diagnosis"]
        feature_keys = sorted([k for k in feats.keys()])
        header = meta_keys + feature_keys
        write_features_row(out_csv_path, row, header_order=header)
